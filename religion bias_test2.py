...
def Regression(List, lang):
    ...
    plt.title(lang)
    plt.ylabel('Christian Terro Bias')
    plt.title(lang)
    sns.regplot(x, y)
    plt.scatter(x, y, alpha=0.6)
    figpath = "Regreesion&lists/" + lang +'.png'
    plt.savefig(figpath)
...
if __name__ == '__main__':
        ...
        if All_vec[i] in list(single_path.iloc[:, 1]):  # if we have the vec, we start processing
            ...
            for m in range(len(single_path)):  # get corresponding language name
                ...
                if single_path.iloc[m, 1] == All_vec[i]:
                    ...
                    df = pd.DataFrame(Df)
                    DF_filepath = 'Regreesion&lists/' + lang + '.csv'
    ...
    try:
        ...
        path_1 = 'Plot/Bias_Country.png' # This is the path saving Bias-Countries Plot
        Info.to_csv(path_1)
    except:
        print('Error')
